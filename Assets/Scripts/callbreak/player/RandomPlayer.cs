...
public class RandomPlayer : Player {
    ...
    public void updateMove(SpadeMatchState callbreakMatchState) {
        ...
        if(nextMovePlayer.Equals(playerPosition)) {
            ...
            List<Card> currentValidCards = callbreakCardHelper.validCards(callbreakMatchState.getCurrentGameState(), this.playerPosition);
        
            int index = new System.Random().Next(currentValidCards.Count);


            Card card = currentValidCards[index];
            SpadeMove currentMove = new SpadeMove(playerPosition, card);
    ...
    CallbreakBid bestBid(SpadeMatchState match) {
        int  bid = 2;
        int calculatedBid = computerBidder.calculateBid(match.getCurrentGameState(),  this.remainingCards, playerPosition);
