...
public class MovePlayedCard : MonoBehaviour
{
    ...
    public IEnumerator playCardEnumerator(SpadeMove callbreakMove)
    {
        ...
        if (!callbreakMove.playerPosition.Equals(PlayerPosition.SOUTH))
        {
            ...
            /* MOVED TO UPDATEMOVE IN GAMEPLAY.
            if(gameState!= null && gameState.gameVariant.Equals(GameVariant.HEARTS) && gameState.isHeartsBroken() && gameState.getHeartsBrokenCard().getCardId().Equals(callbreakMove.card.getCardId())) {
                GameObject.Find("ScriptEmpty").GetComponent<WarningScript>().showWarningAndDisappear(new WarningEntity("<sprite=0> HEARTS broken by : " + callbreakMove.playerPosition, "C"));
